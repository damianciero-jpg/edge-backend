...
function buildScoredPrompt(prompt, evaluation, forceRecommendation) {
  ...
  return [
    ...
    forceRecommendation
      ? '⚠ FORCED RECOMMENDATION MODE: EDGE has passed on 4+ consecutive games and must commit to a pick.'
      : null,
    forceRecommendation
      ? 'You MUST return BET or LEAN — PASS is not permitted. Find the best available play and commit.'
...
function buildMultiCandidatePrompt(basePrompt, pairs, forceRecommendation) {
  ...
  return [
    ...
    '',
    forceRecommendation
      ? '⚠ FORCED RECOMMENDATION: EDGE has passed 4+ times. You MUST pick the single best play — PASS is not allowed.'
      : 'Then identify the single best play from the candidates above, or PASS if none have genuine value after research.',
    '',
