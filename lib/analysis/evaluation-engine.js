...
// ─── MAIN EDGE SCORING ────────────────────────────────────────────────────────
// Weights: priceEdge 50%, teamFormSignal 15%, rlmScore 10%,
//          marketBreadth 10%, confidencePenalty 8%, injurySignal 4%, situationalSignal 3%
//
// rlmScore: Reverse Line Movement — when the line moves opposite to public money,
// sharp money is on the other side. This is the strongest predictive signal we
// can add without a paid public betting data API.
//
// teamFormSignal: real MLB records, streaks, home/away splits (beats keyword matching).
function computeEdgeScore({
  ...
  // HARD GATE: RLM or pitcher edge can provide small override on marginal negative
  if (priceEdge <= 0) {
    const sharpBoost = (rlmScore > 3 ? rlmScore * 0.3 : 0) +
                       (pitcherSignal > 3 ? pitcherSignal * 0.2 : 0);
    return (priceEdge * 0.55) + sharpBoost;
  }

  // Starting pitcher is the single most predictive variable in MLB.
  // When available, it gets its own weight alongside team form.
...
function buildCandidateEvaluation(prompt, candidate, lineMovementScore = 0) {
  ...
  const factors = oddsDetected
    ? getSignalFactors(prompt, odds, opponentOdds, candidate, lineMovementScore, candidate && candidate.pitchers)
    : { marketBreadth: 0, confidencePenalty: 0, injurySignal: 0, situationalSignal: 0, teamFormSignal: 0, rlmScore: 0, pitcherSignal: 0, vigPct: null };
...
function getSignalFactors(prompt, odds, opponentOdds, candidate, rlmScore = 0, pitchers = null) {
  const vigPct = calcVig(odds, opponentOdds);

  // Extract MLB team form from the stats block
  ...
  // Soccer form signal — feeds teamFormSignal slot (MLB block won't match for soccer)
  if (teamFormSignal === 0 && candidate && candidate.soccer && candidate.soccer.homeFormSignal != null) {
    const isHome = candidate.side === 'home';
    // homeFormSignal is from home perspective; negate for away side
    teamFormSignal = isHome ? candidate.soccer.homeFormSignal : -candidate.soccer.homeFormSignal;
  }

  // Extract pitcher quality signal from pitcher data
  let pitcherSignal = 0;
  if (pitchers && pitchers.home && pitchers.away) {
    const isHome = (candidate && candidate.side) === 'home';
    const ourPitcher   = isHome ? pitchers.home : pitchers.away;
    const theirPitcher = isHome ? pitchers.away : pitchers.home;
    // Import calcPitcherQualitySignal dynamically to avoid circular deps
    try {
      const { calcPitcherQualitySignal } = require('./mlb-service');
      pitcherSignal = calcPitcherQualitySignal(ourPitcher, theirPitcher);
    } catch { pitcherSignal = 0; }
  }
