...
function computeEdgeScore({
  ...
  // context signals cannot rescue it — but RLM can provide a small override
  // if the signal is very strong (score > 3) suggesting sharp money disagreement.
  if (priceEdge <= 0) {
    const rlmBoost = rlmScore > 3 ? rlmScore * 0.3 : 0;
    return (priceEdge * 0.55) + rlmBoost;
  }
...
function buildCandidateEvaluation(prompt, candidate, lineMovementScore = 0) {
  ...
  const factors = oddsDetected
    ? getSignalFactors(prompt, odds, opponentOdds, candidate, lineMovementScore)
    : { marketBreadth: 0, confidencePenalty: 0, injurySignal: 0, situationalSignal: 0, teamFormSignal: 0, rlmScore: 0, vigPct: null };
...
function getSignalFactors(prompt, odds, opponentOdds, candidate, rlmScore = 0) {
