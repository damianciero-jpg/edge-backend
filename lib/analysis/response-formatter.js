...
const { getRecommendedAction, percent, roundNumber } = require('./verdict-engine');
...
function buildStructuredResult(evaluation, aiText, forceRecommendation) {
  ...
  const { verdict: consensusVerdict, conflicted, researchOverride, forcedPick } = applyConsensusGate(evaluation.verdict, aiVerdict, evaluation.edgeScore, evaluation.mode, forceRecommendation);

  // If conflicted or forced, override reason appropriately
  ...
  const forcedPickLabel = forcedPick
    ? evaluation.evaluating || (evaluation.allCandidates && evaluation.allCandidates[0]?.label) || 'Best available play'
    : null;
  const finalPick = forcedPick
    ? forcedPickLabel
    : finalVerdict === 'PASS' ? 'PASS — signals conflict' : evaluation.pick;
  ...
  const finalRecommendedAction = conflicted
    ...
    : forcedPick
      ? 'Forced recommendation — EDGE overrode 4+ consecutive passes. Small unit only.'
      : researchOverride
