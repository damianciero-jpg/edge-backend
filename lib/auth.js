...
async function validateOTP(email, otp) {
  ...
  if (USE_KV) {
    ...
    if (stored == null || String(stored).trim() !== String(otp).trim()) return false;
    await redis.del(key);
    return true;
  }
  const entry = localOtpStore.get(key);
  if (!entry || String(entry.otp).trim() !== String(otp).trim() || entry.expires < Date.now()) return false;
...
async function sendOTPEmail(email, otp) {
  if (process.env.RESEND_API_KEY) {
    ...
    await resend.emails.send({
      from: process.env.FROM_EMAIL || 'EDGE <onboarding@resend.dev>',
      to: email,
  ...
  if (process.env.GMAIL_USER && process.env.GMAIL_APP_PASSWORD) {
    ...
    await transporter.sendMail({
      from: process.env.FROM_EMAIL || `"EDGE" <${process.env.GMAIL_USER}>`,
      to: email,
      subject: `${otp} - your EDGE login code`,
      html: buildOTPEmail(otp),
    });
